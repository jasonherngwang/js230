...
function areSchedulesValid(schedules) {
  return (
    schedules.length > 0 &&
    schedules.every(function (schedule) {
...
router.get('/schedules/:staff_id', function (req, res, next) {
  ...
  db.all(
    `SELECT * FROM BOOKINGS WHERE staff_id = ${req.params['staff_id']}`,
    function (err, rows) {
      res.json(rows);
    }
  );
...
router.post('/staff_members', function (req, res, next) {
  ...
  if (!email || !name) {
    ...
    const result = db.run(
      'INSERT INTO STAFFS (name, email) VALUES ($name, $email);',
      {
        $name: name,
        $email: email,
      },
...
router.post('/schedules', function (req, res, next) {
  ...
  if (areSchedulesValid(schedules)) {
    schedules.forEach(function (schedule) {
      db.run(
        'INSERT INTO BOOKINGS (staff_id, date, time) VALUES ($staff_id, $date, $time);',
        {
...
router.post('/bookings', function (req, res, next) {
  ...
  const schedule = db.get(
    'SELECT * FROM BOOKINGS WHERE ID = $id and STUDENT_EMAIL IS NULL;',
    { $id: id },
    function (err, row) {
      if (row) {
        const student = db.get(
          ...
          function (err, row) {
            if (row) {
              db.run(
                'UPDATE BOOKINGS SET STUDENT_EMAIL = $email WHERE ID = $id;',
                { $email: email, $id: id }
              );
              res.status(204).send();
            } else {
              ...
              db.get(
                'INSERT INTO BOOKING_SEQUENCES (student_email, sequence) VALUES ($email, $sequence);',
                { $email: email, $sequence: sequence }
              );
              res
                .status(404)
                .send(`Student does not exist; booking_sequence: ${sequence}`);
            }
          }
        );
      } else {
...
router.post('/students', function (req, res, next) {
  ...
  db.get(
    'SELECT * FROM BOOKING_SEQUENCES WHERE SEQUENCE = $sequence;',
    { $sequence: student_details.booking_sequence },
    function (err, row) {
      if (row) {
        if (student_details.email && student_details.name) {
          db.run('INSERT INTO STUDENTS (name, email) VALUES ($name, $email);', {
...
router.get('/bookings/:date', function (req, res, next) {
  ...
  db.all(
    'SELECT STAFFS.NAME, BOOKINGS.DATE, BOOKINGS.TIME, BOOKINGS.STUDENT_EMAIL FROM BOOKINGS JOIN STAFFS ON BOOKINGS.STAFF_ID = STAFFS.ID;',
    function (err, rows) {
      const booking_details = rows
        .filter(function (schedule) {
...
router.put('/bookings/:booking_id', function (req, res, next) {
  db.get(
    'SELECT * FROM BOOKINGS WHERE ID = $id and STUDENT_EMAIL IS NOT NULL;',
    { $id: req.params['booking_id'] },
    function (err, row) {
      if (row) {
        db.run('UPDATE BOOKINGS SET STUDENT_EMAIL = NULL WHERE ID = $id;', {
          $id: req.params['booking_id'],
        });
        res.sendStatus(204);
      } else {
    ...
    function (err, row) {
      if (row) {
        if (row.student_email) {
          res
            .status(403)
            .send('Can not delete the schedule. There is a booking.');
        } else {
...
router.get('/reset', function (req, res, next) {
  const sql = fs
    .readFileSync(path.join(__dirname, '../db/seed_data.sql'))
    .toString();
