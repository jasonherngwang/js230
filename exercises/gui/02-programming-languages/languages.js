...
document.addEventListener('DOMContentLoaded', () => {
  const languagesList = document.querySelector('#languages_list');
  languages.forEach((language) => {
    ...
    loadLink.textContent = 'Show More';

    const hideLink = document.createElement('a');
    hideLink.textContent = 'Show Less';

    item.addEventListener('click', (e) => {
      e.preventDefault();

      if (e.target === loadLink) {
        p.textContent = language.description;
        loadLink.remove();
        item.appendChild(hideLink);
      } else if (e.target === hideLink) {
        p.textContent = shortText;
        hideLink.remove();
